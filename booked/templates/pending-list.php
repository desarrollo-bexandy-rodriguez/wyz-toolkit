<div class="booked-settings-wrap wrap"><?php
	...
	if (!current_user_can('manage_booked_options') && $booked_none_assigned):
		...
		echo '<div style="text-align:center;">';
			...
			echo '<p>'.esc_html__('Get in touch with the Administration of this site to get a calendar assigned to you.','wyzi-business-finder').'</p>';
		echo '</div>';
		...
		<div class="booked-settings-title"><?php esc_html_e('Pending Appointments','wyzi-business-finder'); ?></div>
	
		<div class="booked-pending-cap bookedClearFix">
			<button class="delete-past button"><?php esc_html_e('Delete Passed Appointments','wyzi-business-finder'); ?></button>
			<button style="float:right; margin-left:10px;" class="approve-all button button-primary"><?php esc_html_e('Approve All','wyzi-business-finder'); ?></button>
			<button style="float:right;" class="delete-all button"><?php esc_html_e('Delete All','wyzi-business-finder'); ?></button>
		</div><?php
		...
		echo '<div class="booked-pending-appt-list">';
			...
			*/
				...
				foreach($appointments_array as $appt):
					...
					echo '<div class="pending-appt bookedClearFix'.($late_date > $date_to_compare ? ' passed' : '').'" data-appt-id="'.$appt['post_id'].'">';
						...
						echo '<span class="appt-block" data-appt-id="'.$appt['post_id'].'">';
							...
							echo '<button data-appt-id="'.$appt['post_id'].'" class="approve button button-primary">'.esc_html__('Approve','wyzi-business-finder').'</button>';
							echo '<button class="delete button">'.esc_html__('Delete','wyzi-business-finder').'</button>';
		
							echo $user_display;
							
							echo '<br>';
							if ($late_date > $date_to_compare): echo '<span class="late-appt">' . esc_html__('This appointment has passed.','wyzi-business-finder') . '</span><br>'; endif;
							if ($calendar_owner_name): echo '<i class="booked-icon booked-icon-user"></i>&nbsp;&nbsp;'.sprintf(esc_html__('Assigned to %s','wyzi-business-finder'),$calendar_owner_name).'<br>'; endif;
			...
			echo '<div class="pending-appt'.(!empty($appointments_array) ? ' no-pending-message' : '').'">';
				echo '<p style="text-align:center; margin:3em 0 2em">'.esc_html__('There are no pending appointments.','wyzi-business-finder').'</p>';
			echo '</div>';

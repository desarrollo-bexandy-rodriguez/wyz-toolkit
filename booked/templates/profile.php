...
if (empty($user_data)) {
	...
	echo '<h2>' . esc_html__('No profile here!','wyzi-business-finder') . '</h2>';
	echo '<p>' . esc_html__('Sorry, this user profile does not exist.','wyzi-business-finder') . '</p>';
		...
		<div class="booked-info">
			<div class="booked-user">
				...
				<h3 class="<?php echo $h3_class; ?>">
					<?php echo sprintf(esc_html__('Welcome back, %s!','wyzi-business-finder'),'<strong>'.booked_get_name( $user_data->ID ).'</strong>'); ?>
					<?php if ($my_profile): ?>
						&nbsp;&nbsp;<a class="booked-logout-button" href="<?php echo wp_logout_url(get_permalink($post->ID)); ?>" title="<?php esc_html_e('Sign Out','wyzi-business-finder'); ?>"><?php esc_html_e('Sign Out','wyzi-business-finder'); ?></a>
					<?php endif; ?>
				</h3>
			</div>
		</div>
	...
	<ul class="booked-tabs bookedClearFix">
		<?php
			...
			$default_tabs = array(
				'appointments' => array(
					'title' => esc_html__('Upcoming Appointments','wyzi-business-finder'),
					'booked-icon' => 'booked-calendar',
					'class' => false
				),
				'history' => array(
					'title' => esc_html__('Appointment History','wyzi-business-finder'),
					'booked-icon' => 'booked-calendar-check-o',
					'class' => false
				),
				'edit' => array(
					'title' => esc_html__('Edit Profile','wyzi-business-finder'),

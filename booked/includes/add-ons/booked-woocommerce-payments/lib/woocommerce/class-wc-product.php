...
class Booked_WC_Product {
	...
	public static function get( $post_id = null ) {
		if ( !is_integer($post_id) ) {
			$message = sprintf( __('%s integer expected when %s given.', 'wyzi-business-finder'), 'Booked_WC_Product::get($post_id)', gettype($post_id) );
	...
	protected function _get_product_data() {
		...
		if ( !$this->data ) {
			$message = sprintf(__('An error has occur while retrieving product data for product with ID %1$d.', 'wyzi-business-finder'), $this->post_id);
			throw new Exception($message);
		}

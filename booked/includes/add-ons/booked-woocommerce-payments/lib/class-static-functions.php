...
class Booked_WC_Functions {
	...
	public static function get_products() {
		...
		if (
			...
			$quer = array(
				'posts_per_page' => -1, // there shouldn't be more than 200 product
				'post_type' => 'product',
				'post_status' => 'publish',
				'meta_query' => array(
					array(
			...
			if ( ! current_user_can( 'manage_options' ) ) {
				$quer['author'] = get_current_user_id();
				if ( is_page( 'user-account' ) && isset( $_GET['business-calendar'] ) ) {
					$post = get_post( $_GET['business-calendar'] );
					if ( null != $post && get_current_user_id() == $post->post_author ) {
						$quer['meta_query'][] = array(
							'key' => 'business_id',
							'value' => $_GET['business-calendar'],
							'compare' => '=',
						);
						$quer['relation'] = 'AND';
					}
			...
			if ( $calendar_id ) {
				...
				if ( $assignee_email && ($usr=get_user_by('email', $assignee_email)) ) {
					$custom_field_data['booking-agent'] = array(
						'label' => esc_attr__('Booking Agent', 'wyzi-business-finder'),
	...
	public static function booked_fea_shortcode_appointments_buttons($button_html, $appt_id) {
		...
		if ($appointment->order_id):
			...
			$button_html = '<div class="booked-fea-buttons">';
				$button_html .= ($status_class == 'pending' ? '<button data-appt-id="'.$appt_id.'" class="approve button button-primary">'.__('Approve','wyzi-business-finder').'</button>' : '');
			$button_html .=	'</div>';
			return $button_html;
		else:
			return $button_html;
	...
	public static function booked_button_book_appointment( $label ) {
		...
		return '<span class="button-text" >' . __('Choose New Date', 'wyzi-business-finder') . '</span></button></span>';
	}

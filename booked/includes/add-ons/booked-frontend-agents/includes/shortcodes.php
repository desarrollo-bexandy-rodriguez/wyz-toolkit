		...
		if (is_user_logged_in() && current_user_can('edit_booked_appointments')):
			...
			if (!isset($atts['remove_wrapper'])): echo '<div id="booked-profile-page" class="booked-shortcode">'; endif;
				...
				echo '<div class="booked-fea-appt-list">';
					...
					if ($pending):
						if ($total_appts):
							echo '<h4>'.sprintf(_n('%s Pending Appointment','%s Pending Appointments',$total_appts,'wyzi-business-finder'),'<span class="count">'.number_format($total_appts).'</span>').'</h4>';
						else:
							echo '<p class="booked-no-margin">'.__('No pending appointments.','wyzi-business-finder').'</p>';
						endif;
					elseif ($historic):
						if ($total_appts):
							echo '<h4>'.sprintf(_n('%s Past Appointment','%s Past Appointments',$total_appts,'wyzi-business-finder'),'<span class="count">'.number_format($total_appts).'</span>').'</h4>';
						else:
							echo '<p class="booked-no-margin">'.__('No past appointments.','wyzi-business-finder').'</p>';
						endif;	
					else:
						if ($total_appts):
							echo '<h4>'.sprintf(_n('%s Upcoming Appointment','%s Upcoming Appointments',$total_appts,'wyzi-business-finder'),'<span class="count">'.number_format($total_appts).'</span>').'</h4>';
						else:
							echo '<p class="booked-no-margin">'.__('No upcoming appointments.','wyzi-business-finder').'</p>';
						endif;
					endif;
					...
					foreach($appointments_array as $appt):
						...
						if ($date_display == $today){
							$date_display = __('Today','wyzi-business-finder');
							$day_name = '';
						} else {
						...
						if ($timeslots[0] == '0000' && $timeslots[1] == '2400'):
							...
							$timeslotText = (!get_option('booked_hide_end_times') ? __('from','wyzi-business-finder').' ' : __('at','wyzi-business-finder').' ') . $time_start . (!get_option('booked_hide_end_times') ? ' ' . __('to','wyzi-business-finder').' '.$time_end : '');
							$google_date_startend_end = $google_date_startend;
						endif;

						$status = ($appt['status'] !== 'publish' && $appt['status'] !== 'future' ? __('pending','wyzi-business-finder') : __('approved','wyzi-business-finder'));
						...
						echo '<div class="appt-block bookedClearFix" data-appt-id="'.$appt['post_id'].'">';
							...
							$default_button_html = '<div class="booked-fea-buttons">';
								...
								$default_button_html .= ($status_class == 'pending' ? '<button data-appt-id="'.$appt['post_id'].'" class="approve button button-primary">'.__('Approve','wyzi-business-finder').'</button>' : '');
							...
							if (!isset($appt['guest_name'])):
								...
								if (isset($user_info->ID)):
									...
									$user_display = __('(this user no longer exists)','wyzi-business-finder');
								endif;
							else :
							...
							echo $user_display;
							echo '<br>';
							if ($late_date > $date_to_compare): echo '<span class="late-appt">' . __('This appointment has passed.','wyzi-business-finder') . '</span><br>'; endif;
							...
							echo ($cf_meta_value ? '<br><i class="booked-icon booked-icon-info"></i><a href="#" class="booked-show-cf">'.__('Additional information','wyzi-business-finder').'</a><div class="cf-meta-values-hidden">'.$cf_meta_value.'</div>' : '');
			...
			return '<p>'.__('Please log in to view your upcoming appointments.','wyzi-business-finder').'</p>';

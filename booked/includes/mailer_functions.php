...
function booked_mailer_tokens(){
	return apply_filters( 'booked_mailer_tokens', array(
		'name' => esc_html__( "Display the full name of the customer.",'wyzi-business-finder'),
		'email' => esc_html__( "Display the customer's email address.",'wyzi-business-finder'),
		'title' => esc_html__( "Display the title of the appointment's time slot.",'wyzi-business-finder'),
		'calendar' => esc_html__( "Display the appointment's calendar name (if applicable).",'wyzi-business-finder'),
		'date' => esc_html__( "Display the appointment date.",'wyzi-business-finder'),
		'time' => esc_html__( "Display the appointment time.",'wyzi-business-finder'),
		'customfields' => esc_html__( "Display the appointment's custom field data.",'wyzi-business-finder'),
		'id' => esc_html__( "Display the appointment's unique identification number.",'wyzi-business-finder'),
	));
}

function booked_user_tokens(){
	return apply_filters( 'booked_user_tokens', array(
		'name' => esc_html__( "Display the customer's name.",'wyzi-business-finder'),
		'username' => esc_html__( "Display the customer's username.",'wyzi-business-finder'),
		'password' => esc_html__( "Display the customer's password.",'wyzi-business-finder'),
		'email' => esc_html__( "Display the customer's email address.",'wyzi-business-finder')
	));
}
...
function booked_get_appointment_tokens( $appt_id ){
	...
	if ($timeslots[0] == '0000' && $timeslots[1] == '2400'):
		$time_text = esc_html__( 'All day', 'wyzi-business-finder');
	else :

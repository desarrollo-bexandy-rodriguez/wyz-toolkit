...
class BookedShortcodes {
	...
	public function booked_registration_redirect(){
		...
		if ( get_option('users_can_register') && !is_user_logged_in() && isset($_POST['booked_reg_submit'] ) ) {
			...
			if ( $name_requirements == 'require_surname' && isset($_POST['booked_reg_surname']) && !$_POST['booked_reg_surname'] ):
				...
				$booked_reg_errors[] = esc_html__('A first and last name are required to register.','wyzi-business-finder');
				
			elseif ( !isset($_POST['booked_reg_name']) || isset($_POST['booked_reg_name']) && !$_POST['booked_reg_name'] ):
			
				$registration_complete = 'error';
				$booked_reg_errors[] = esc_html__('A name is required to register.','wyzi-business-finder');
    ...
    public function booked_calendar_switcher_shortcode( $attrs ){
		...
		if (!get_option('booked_hide_default_calendar')): $args['show_option_all'] = esc_html__('Default Calendar','wyzi-business-finder'); endif;
		...
		if ( is_user_logged_in() ):
			...
			if (!isset($atts['remove_wrapper'])): echo '<div id="booked-profile-page" class="booked-shortcode">'; endif;
				...
				echo '<div class="booked-profile-appt-list">';
					...
					if ($historic):
						if ($total_appts):
							echo '<h4><span class="count">' . number_format($total_appts) . '</span> ' . _n('Past Appointment','Past Appointments',$total_appts,'wyzi-business-finder') . '</h4>';
						else:
							echo '<p class="booked-no-margin">'.esc_html__('No past appointments.','wyzi-business-finder').'</p>';
						endif;
					else:
						if ($total_appts):
							echo '<h4><span class="count">' . number_format($total_appts) . '</span> ' . _n('Upcoming Appointment','Upcoming Appointments',$total_appts,'wyzi-business-finder') . '</h4>';
						else:
							echo '<p class="booked-no-margin">'.esc_html__('No upcoming appointments.','wyzi-business-finder').'</p>';
						endif;
					endif;
					...
					foreach($appointments_array as $appt):
						...
						if ($date_display == $today){
							$date_display = esc_html__('Today','wyzi-business-finder');
							$day_name = '';
						} else {
						...
						$timeslotText = '';
						$status = ($appt['status'] != 'publish' && $appt['status'] != 'future' ? esc_html__('pending','wyzi-business-finder') : esc_html__('approved','wyzi-business-finder'));
						...
						if ($timeslots[0] == '0000' && $timeslots[1] == '2400'):
							...
							if ($only_titles && !$ts_title || !$only_titles):
								$timeslotText = (!get_option('booked_hide_end_times') ? esc_html__('from','wyzi-business-finder').' ' : esc_html__('at','wyzi-business-finder').' ') . $time_start . (!get_option('booked_hide_end_times') ? ' &ndash; '.$time_end : '');
							endif;
						...
						echo '<span class="appt-block bookedClearFix '.(!$historic ? $status_class : 'approved').'" data-appt-id="'.$appt['post_id'].'">';
							...
							echo (!empty($appt['calendar_id']) ? '<i class="booked-icon booked-icon-calendar"></i><strong>'.esc_html__('Calendar','wyzi-business-finder').':</strong> '.$appt['calendar_id'][0]->name.'<br>' : '');
							...
							echo ($cf_meta_value ? '<br><i class="booked-icon booked-icon-info"></i><a href="#" class="booked-show-cf">'.esc_html__('Additional information','wyzi-business-finder').'</a><div class="cf-meta-values-hidden">'.$cf_meta_value.'</div>' : '');
							...
							if (!$historic):
								...
								if ( apply_filters('booked_shortcode_appointments_allow_cancel', true, $appt['post_id']) && !get_option('booked_dont_allow_user_cancellations',false) ) { if ( $appt_date_time >= $date_to_compare ) { echo '<a href="#" data-appt-id="'.$appt['post_id'].'" class="cancel">'.esc_html__('Cancel Appointment','wyzi-business-finder').'</a>'; } }
			...
			return '<p>'.esc_html__('Please log in to view your upcoming appointments.','wyzi-business-finder').'</p>';
			...
			?><div id="booked-profile-page">
				...
				<div id="booked-page-form">
					...
					<ul class="booked-tabs login bookedClearFix">
						<li<?php if ( !isset($_POST['booked_reg_submit'] ) ) { ?> class="active"<?php } ?>><a href="#login"><i class="booked-icon booked-icon-lock"></i><?php esc_html_e('Sign In','wyzi-business-finder'); ?></a></li>
						<?php if ( get_option('users_can_register') ): ?><li<?php if ( isset($_POST['booked_reg_submit'] ) ) { ?> class="active"<?php } ?>><a href="#register"><i class="booked-icon booked-icon-pencil"></i><?php esc_html_e('Register','wyzi-business-finder'); ?></a></li><?php endif; ?>
						<li><a href="#forgot"><i class="booked-icon booked-icon-question-circle"></i><?php esc_html_e('Forgot Password','wyzi-business-finder'); ?></a></li>
					</ul>
					...
					<div id="profile-login" class="booked-tab-content">
						...
						<?php if (isset($reset) && $reset == true) { ?>
							...
							<strong><?php esc_html_e('Success!','wyzi-business-finder'); ?></strong><br />
							<?php esc_html_e('Check your email to reset your password.','wyzi-business-finder'); ?>
							</p>
						...
						<div class="booked-form-wrap bookedClearFix">
							<div class="booked-custom-error"><?php esc_html_e('Both fields are required to log in.','wyzi-business-finder'); ?></div>
							<?php if (isset($_GET['loginfailed'])): ?><div class="booked-custom-error not-hidden"><?php esc_html_e('Sorry, those login credentials are incorrect.','wyzi-business-finder'); ?></div><?php endif; ?>
							...
							<?php echo wp_login_form( array( 'echo' => false, 'redirect' => get_the_permalink($login_redirect), 'label_username' => esc_html__( 'Email Address','wyzi-business-finder') ) ); ?>
						</div>
					</div>
					...
					<div id="profile-forgot" class="booked-tab-content">
						<div class="booked-form-wrap bookedClearFix">
							<div class="booked-custom-error"><?php esc_html_e('An email address is required to reset your password.','wyzi-business-finder'); ?></div>
							<form method="post" action="<?php echo site_url('wp-login.php?action=lostpassword', 'login_post') ?>" class="wp-user-form">
								<p class="username">
									<label for="user_login"><?php esc_html_e('What is your email address?','wyzi-business-finder'); ?></label>
									<input type="text" name="user_login" value="" size="20" id="user_login" tabindex="1001" />
								</p>

								<?php do_action('login_form', 'resetpass'); ?>
								<input type="submit" name="user-submit" value="<?php esc_html_e('Reset my password','wyzi-business-finder'); ?>" class="user-submit button-primary" tabindex="1002" />

...
function booked_add_to_calendar_button($dates,$cf_meta_value){
	if (!get_option('booked_hide_google_link',false)):
		...
		?><span class="addtocalendar atc-style-booked google-cal-button">
			<a class="atcb-link"><?php esc_html_e('Add to Calendar','wyzi-business-finder'); ?></a>
	        <var class="atc_event">
	            ...
	            <var class="atc_title"><?php echo sprintf(esc_html__('Appointment with %s','wyzi-business-finder'),get_bloginfo('name')); ?></var>
...
function booked_filter_business_calendars($user_id,$business_id,$calendars)
{
	$user_calendars = get_user_meta( $user_id, 'wyz_business_calendars', true );
	if ( empty( $calendars ) || ! isset( $user_calendars[ $business_id ] ) ) return array();
	$business_calendars = array();
	foreach ($calendars as $calendar) {
		if ( $calendar->term_id == $user_calendars[ $business_id ] )
			$business_calendars[] = $calendar;
	}
...
function booked_registration_validation( $email, $password, $captcha_value = false, $captcha_from_user = false )  {
	...
	if ($captcha_value):
		if ($captcha_value != $captcha_from_user):
			$reg_errors->add('captcha', esc_html__('The text you\'ve entered does not match the image.','wyzi-business-finder'));
		else :
	...
	if ( !$email || !$password ) {
	    $reg_errors->add('field', esc_html__('All fields are required to register.','wyzi-business-finder'));
	}

	if ( username_exists( $email ) ) {
    	$reg_errors->add('user_name', esc_html__('That username already exists.','wyzi-business-finder'));
    }

    if ( ! validate_username( $email ) ) {
	    $reg_errors->add( 'username_invalid', esc_html__('That name is not valid.','wyzi-business-finder'));
	}

    if ( !is_email( $email ) ) {
	    $reg_errors->add( 'email_invalid', esc_html__('That email address is not valid.','wyzi-business-finder'));
	}

	if ( email_exists( $email ) ) {
	    $reg_errors->add( 'email', esc_html__('That email is already in use.','wyzi-business-finder'));
	}
...
function booked_complete_registration() {
    ...
    if ( 1 > count( $reg_errors->get_error_messages() ) ) {
        ...
        return '<p class="booked-form-notice"><strong>'.esc_html__('Success!','wyzi-business-finder').'</strong><br />'.esc_html__('Registration complete, please check your email for login information.','wyzi-business-finder').'</p>';
...
function booked_registration_form($name, $surname, $email, $password){
	...
	?><form action="" method="post" class="wp-user-form">
		...
		<p class="name">
			<label for="name"><?php echo ( $name_requirements == 'require_surname' ? esc_html__('First Name','wyzi-business-finder') : esc_html__('Your Name','wyzi-business-finder') ); ?></label>
		...
		<p class="surname">
			<label for="surname"><?php esc_html_e('Last Name','wyzi-business-finder'); ?></label>
		...
		<p class="password">
			<label for="password"><?php esc_html_e('Choose a Password','wyzi-business-finder'); ?></label>
		...
		<?php if (class_exists('ReallySimpleCaptcha')) :
			...
			?><p class="captcha">
				<label for="captcha_code"><?php esc_html_e('Please enter the following text:','wyzi-business-finder'); ?></label><?php
		...
		<input type="submit" name="booked_reg_submit" value="<?php esc_html_e('Register','wyzi-business-finder'); ?>" class="user-submit button-primary" tabindex="105" />

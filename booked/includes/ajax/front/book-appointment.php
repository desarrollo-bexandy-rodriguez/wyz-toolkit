...
if ($appt_is_available):
	...
	if ($customer_type == 'guest'):
		...
		if ( $name_requirements == 'require_surname' && !$surname ):
			...
			echo 'error###'.esc_html__('Your full name is required to book an appointment.','wyzi-business-finder');
			...
			if ($email && is_email($email) && $name || !$email && !$email_required && $name):
				...
				if ($email && !is_email($email)):
					$errors[] = esc_html__('The email address you have entered doesn\'t appear to be valid.','wyzi-business-finder');
				elseif ($email_required && !$email):
					$errors[] = esc_html__('Your name and a valid email address are required to book an appointment.','wyzi-business-finder');
				elseif (!$name):
					$errors[] = esc_html__('Your name is required to book an appointment.','wyzi-business-finder');
				else:
					$errors[] = esc_html__('An unknown error occured.','wyzi-business-finder');
				endif;
...
elseif ( $is_new_registration ):
	...
	if ( $name_requirements == 'require_surname' && !$surname ):
		...
		if (empty($errors)):
	        ...
	        echo 'success###' . esc_html__('Registration has been successful.','wyzi-business-finder');
...
',$errors);
	...
	$error_message = apply_filters( 
		...
		esc_html__('Sorry, someone just booked this appointment before you could. Please choose a different booking time.','wyzi-business-finder')
	);

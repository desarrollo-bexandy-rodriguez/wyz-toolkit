...
function booked_timeslots_select( $appt_id = false, $year, $month, $day = false ){
	...
	if ($timeslots[0] == '0000' && $timeslots[1] == '2400'):
		...
		$timeslotText = $time_start.' '.esc_html__('to','wyzi-business-finder').' '.$time_end;
	endif;
		...
		if ( !empty($available_timeslots_array) ):
			foreach( $available_timeslots_array as $_timeslot => $_available ):
				...
				if ( $timeslot == $_timeslot ):
					$_available = '';
				elseif ( $_available > 0 ):
					$_available = ' (' . sprintf( _n( '%d Space Available', '%d Spaces Available', $_available, 'wyzi-business-finder'), $_available ) . ')';
				else:
					$_available = ' [' . esc_html__('Full','wyzi-business-finder') . ']';
				endif;
				...
				if ($_timeslots[0] == '0000' && $_timeslots[1] == '2400'):
					$_timeslotText = esc_html__('All day','wyzi-business-finder');
				else :
					$_timeslotText = sprintf( esc_html__('%s to %s','wyzi-business-finder'), $_time_start, $_time_end );
				endif;
			...
			echo '<option value="' . $timeslot . '">' . $timeslotText . ' [' . esc_html__('Full','wyzi-business-finder') . ']</option>';
		endif;
...
function booked_parse_readme_changelog(){
	...
	$readme = preg_replace( '/[\040]\*\*\NEW\*\*/', '<strong class="new">' . esc_html__( 'New', 'wyzi-business-finder') . '</strong>', $readme);
	$readme = preg_replace( '/[\040]\*\FIX\*/', '<em class="fix">' . esc_html__( 'Fixed', 'wyzi-business-finder') . '</em>', $readme);
	$readme = preg_replace( '/[\040]\*\*(.*?)\*\*/', '<strong>\\1</strong>', $readme);
	$readme = preg_replace( '/[\040]\*(.*?)\*/', '<em>\\1</em>', $readme);
	$readme = explode( '== Changelog ==', $readme );
	$readme = explode( '== Upgrade Notice ==', $readme[1] );
	$readme = $readme[0];
	$whats_new_title = '<h3>'. esc_html__( "What's new in this update?", 'wyzi-business-finder') . '</h3>';
...
function booked_render_single_timeslot_form($timeslot_intervals,$type = false){
	...
	echo '<form id="single-timeslot-form" action="" method="post">';
		...
		echo booked_render_text_field('title', esc_html__('Title (optional)','wyzi-business-finder'));
		echo booked_render_time_select('startTime',$timeslot_intervals,esc_html__('Start time ...','wyzi-business-finder'),true);
		echo booked_render_time_select('endTime',$timeslot_intervals,esc_html__('End time ...','wyzi-business-finder'));
...
function booked_render_bulk_timeslot_form($timeslot_intervals,$type = false){
	...
	echo '<form id="bulk-timeslot-form" action="" method="post">';
		...
		echo booked_render_text_field('title', esc_html__('Title (optional)','wyzi-business-finder'));
		echo booked_render_time_select('startTime',$timeslot_intervals,esc_html__('Start time ...','wyzi-business-finder'));
		echo booked_render_time_select('endTime',$timeslot_intervals,esc_html__('End time ...','wyzi-business-finder'));
		booked_render_time_between_select('time_between',esc_html__('Time between ...','wyzi-business-finder'));
		booked_render_interval_select('interval',esc_html__('Appt Length ...','wyzi-business-finder'));
		booked_render_count_select('count',esc_html__('# of Each ...','wyzi-business-finder'));
...
function booked_render_time_select($select_name,$interval,$placeholder,$single = false){
	...
	$html .= '<option value="">'.$placeholder.'</option>';
		if ($single): $html .= '<option value="allday">'.esc_html__('All Day','wyzi-business-finder').'</option>'; endif;
		do {
		...
		$html .= '<option value="2400">'.date_i18n($time_format,strtotime('2014-01-01 24:00')).' ('.esc_html__('night','wyzi-business-finder').')</option>';
	$html .= '</select>';
...
function booked_render_timeslots($calendar_id = false){
	...
	$day_loop = array(
		esc_html__('Sunday','wyzi-business-finder'),
		esc_html__('Monday','wyzi-business-finder'),
		esc_html__('Tuesday','wyzi-business-finder'),
		esc_html__('Wednesday','wyzi-business-finder'),
		esc_html__('Thursday','wyzi-business-finder'),
		esc_html__('Friday','wyzi-business-finder'),
		esc_html__('Saturday','wyzi-business-finder')
	);
		...
		<tr>
			...
			<td>
				<table>
					<thead>
						<tr>
							<?php echo '<th data-day="'.$day_loop_english_array[$key].'">'.$day.'<a href="#" class="booked-add-timeslot button">'.esc_html__('Add','wyzi-business-finder').'...</a></th>'; ?>
						</tr>
					</thead>
					<tbody>
						<tr>
						...
						<tr>
							<?php echo '<td class="dayTimeslots" data-day="'.$day_loop_english_array[$key].'">';
								if (!empty($booked_defaults[$day_loop_english_array[$key]])):
									...
									echo '<p><small>'.esc_html__('No time slots.','wyzi-business-finder').'</small></p>';
								endif;
							echo '</td>'; ?>
						</tr>
					</tbody>
...
function booked_render_timeslot_info($time_format,$day,$time,$count,$calendar_id,$booked_defaults=array()){
	...
	echo '<span class="timeslot" data-timeslot="'.$time.'">';
		...
		if ($time[0] == '0000' && $time[1] == '2400'):
			$timeslotText = '<span class="start">' . strtoupper( esc_html__( 'All day', 'wyzi-business-finder') ) . '</span>';
		else :
		...
		echo '<span class="slotsBlock">';
			...
			echo '<span class="count"><em>'.$count.'</em> '._n('Space Available','Spaces Available',$count,'wyzi-business-finder').'</span>';
...
function booked_render_interval_select($select_name,$placeholder){
	...
	<option value="60" selected><?php esc_html_e('Every 1 hour','wyzi-business-finder'); ?></option>
	<option value="90"><?php esc_html_e('Every 1 hour, 30 minutes','wyzi-business-finder'); ?></option>
	<option value="120"><?php esc_html_e('Every 2 hours','wyzi-business-finder'); ?></option>
	<option value="45"><?php esc_html_e('Every 45 minutes','wyzi-business-finder'); ?></option>
	<option value="30"><?php esc_html_e('Every 30 minutes','wyzi-business-finder'); ?></option>
	<option value="20"><?php esc_html_e('Every 20 minutes','wyzi-business-finder'); ?></option>
	<option value="15"><?php esc_html_e('Every 15 minutes','wyzi-business-finder'); ?></option>
	<option value="10"><?php esc_html_e('Every 10 minutes','wyzi-business-finder'); ?></option>
	<option value="5"><?php esc_html_e('Every 5 minutes','wyzi-business-finder'); ?></option>
	<?php echo '</select>';
...
function booked_render_time_between_select($select_name,$placeholder){
	...
	<option value="5"><?php esc_html_e('5 minutes','wyzi-business-finder'); ?></option>
	<option value="10"><?php esc_html_e('10 minutes','wyzi-business-finder'); ?></option>
	<option value="15"><?php esc_html_e('15 minutes','wyzi-business-finder'); ?></option>
	<option value="20"><?php esc_html_e('20 minutes','wyzi-business-finder'); ?></option>
	<option value="30"><?php esc_html_e('30 minutes','wyzi-business-finder'); ?></option>
	<option value="45"><?php esc_html_e('45 minutes','wyzi-business-finder'); ?></option>
	<option value="60"><?php esc_html_e('1 hour','wyzi-business-finder'); ?></option>
	<?php echo '</select>';
...
function booked_render_count_select($select_name,$placeholder){
	...
	echo '<select name="'.$select_name.'">';
		do {
			$counter++;
			echo '<option value="'.$counter.'"'.($counter == 1 ? ' selected="selected"' : '').'>'.sprintf(_n('%d space available','%d spaces available',$counter,'wyzi-business-finder'),$counter).'</option>';		
...
function booked_admin_calendar($year = false,$month = false,$calendar_id = false,$size = false){
	...
	?><table class="booked-calendar<?php echo ($size ? ' '.$size : ''); ?>"<?php echo ($calendar_id ? ' data-calendar-id="'.$calendar_id.'"' : ''); ?> data-monthShown="<?php echo $monthShown; ?>">
		<thead>
				...
				<th colspan="<?php if (!$hide_weekends): ?>7<?php else: ?>5<?php endif; ?>">
					...
					<span class="monthName">
						<?php if ($monthShown != $currentMonth): ?>
							<a href="#" class="backToMonth" data-goto="<?php echo $currentMonth; ?>"><i class="booked-icon booked-icon-arrow-left"></i>&nbsp;&nbsp;&nbsp;<?php esc_html_e('Back to','wyzi-business-finder'); ?> <?php echo date_i18n('F',strtotime($currentMonth)); ?></a>
						<?php endif; ?>
			...
			<tr class="days">
				<?php if ($first_day_of_week == 7 && !$hide_weekends): echo '<th>'.esc_html__('Sun','wyzi-business-finder').'</th>'; endif; ?>
				<th><?php esc_html_e('Mon','wyzi-business-finder'); ?></th>
				<th><?php esc_html_e('Tue','wyzi-business-finder'); ?></th>
				<th><?php esc_html_e('Wed','wyzi-business-finder'); ?></th>
				<th><?php esc_html_e('Thu','wyzi-business-finder'); ?></th>
				<th><?php esc_html_e('Fri','wyzi-business-finder'); ?></th>
				<?php if (!$hide_weekends): ?><th><?php esc_html_e('Sat','wyzi-business-finder'); ?></th><?php endif; ?>
				<?php if ($first_day_of_week == 1 && !$hide_weekends): echo '<th>'.esc_html__('Sun','wyzi-business-finder').'</th>'; endif; ?>
			</tr>
		</thead>
		<tbody><?php
				...
				for($x = 1; $x <= 7; $x++){
					...
					if ($hide_weekends && $day_of_week == 6 || $hide_weekends && $day_of_week == 7):
						...
						echo '<td data-date="'.$check_year.'-'.$check_month.'-'.$date.'" class="'.implode(' ',$classes).'">';
							echo '<span class="date'.($appointments_count > 0 ? ' tooltip' : '').'" '.($appointments_count > 0 ? ' title="'.sprintf(_n('%d Appointment','%d Appointments',$appointments_count,'wyzi-business-finder'),$appointments_count).'"' : '').'><span class="number">'. $date . '</span></span>';
						echo '</td>';
		...
		if ($tabbed):
			...
			?><ul id="bookedAppointmentTabs" class="bookedClearFix">
				...
				if ( !get_option('booked_hide_default_calendar') && current_user_can('manage_booked_options') ):
					...
					?><li<?php if (!$calendar_id): ?> class="active"<?php endif; ?>><a href="#calendar-default"><?php esc_html_e('Default Calendar','wyzi-business-finder'); ?><?php echo ($total_appointments ? '<span>'.$total_appointments.'</span>' : ''); ?></a></li><?php
					$is_first_tab = false;
				endif;
				foreach($calendars as $calendar):
			...
			$tab_title = esc_html__('Appointments for','wyzi-business-finder');
			$is_first_tab = true;
			if ( !get_option('booked_hide_default_calendar') && current_user_can('manage_booked_options') ):
			...
			foreach($calendars as $calendar):
				?><div id="bookedCalendarAppointmentsTab-<?php echo $calendar->term_id; ?>" class="bookedAppointmentTab<?php if ( $calendar_id == $calendar->term_id || !$calendar_id && $is_first_tab): ?> active<?php endif; ?>"><?php
					...
					$tab_title = sprintf(esc_html__('%s Appointments for','wyzi-business-finder'),'<strong>'.$calendar->name.'</strong>');
...
function booked_admin_calendar_date_loop($date,$time_format,$date_format,$calendar_id = false,$tab_title,$tabbed = false,$calendars = false){
	...
	if ($todays_defaults){
			...
			*/
			...
			echo '<div class="timeslot bookedClearFix' . ( $title ? ' has-title ' : '' ) . ( $is_disabled ? ' booked-disabled' : '' ) . '">';
				...
				echo '<span class="timeslot-count">';
					...
					echo '<span class="spots-available'.($spots_available == 0 ? ' empty' : '').'">'.$spots_available.' '._n('space','spaces',$spots_available,'wyzi-business-finder').' '.esc_html__('available','wyzi-business-finder').'</span>';
					...
					*/
					...
					if (!empty($appts_in_this_timeslot)):
						...
						echo '<strong>'. sprintf( esc_html( _n('%d Appointment','%d Appointments',$booked_appts,'wyzi-business-finder') ), $booked_appts ) . ':</strong>';
						...
						foreach($appts_in_this_timeslot as $appt_id):
							...
							if (!isset($appointments_array[$appt_id]['guest_name'])):
								...
								if (isset($user_info->ID)):
									...
									$user_display = esc_html__('(this user no longer exists)','wyzi-business-finder');
								endif;
							else :
							...
							echo '<span class="appt-block" data-appt-id="'.$appt_id.'">';
								...
								if ( apply_filters('booked_admin_show_calendar_buttons', true) ) {
									echo '<a href="#" class="delete"'.($calendar_id ? ' data-calendar-id="'.$calendar_id.'"' : '').'><i class="booked-icon booked-icon-close"></i></a>'.($status == 'pending' ? '<button data-appt-id="'.$appt_id.'" class="approve button button-primary">'.esc_html__('Approve','wyzi-business-finder').'</button>' : '');
								}
				...
				echo '<span class="timeslot-people">';
					echo '<button data-timeslot="'.$timeslot.'" data-title="'.esc_attr($title).'" data-date="'.$date.'"'.($calendar_id ? ' data-calendar-id="'.$calendar_id.'"' : '').' class="new-appt button button-primary"'.(!$spots_available ? ' disabled' : '').'>'.esc_html__('New Appointment','wyzi-business-finder').'</button>';
					echo ( empty($appts_in_this_timeslot) ? '<button data-timeslot="'.$timeslot.'" data-title="'.esc_attr($title).'" data-date="'.$date.'"'.($calendar_id ? ' data-calendar-id="'.$calendar_id.'"' : '').' class="disable-slot button"'.(!$spots_available ? ' disabled' : '').'>' . ( $is_disabled ? esc_html__('Enable','wyzi-business-finder') : esc_html__('Disable','wyzi-business-finder') ) . '</button>' : '' );
				echo '</span>';
			echo '</div>';
		...
		*/
		...
		if (!empty($appointments_array)):
			...
			echo '<br><p>'.esc_html__('There are additional appointments booked from previously available time slots:','wyzi-business-finder').'</p>';
	...
	*/
		...
		echo '<span class="additional-timeslots">';
		echo '<p>'.esc_html__('There are no appointment slots available for this day, however there are appointments booked from previously available time slots:','wyzi-business-finder').'</p>';
		foreach($appointments_array as $appointment):
	...
	*/
		...
		echo '<p>'.esc_html__('There are no appointment time slots available for this day.','wyzi-business-finder').' <a href="'.get_admin_url(null,'admin.php?page=booked-settings#defaults').'">'.esc_html__('Would you like to add some?','wyzi-business-finder').'</a></p>';
	}
}
...
function booked_admin_calendar_date_square($date,$calendar_id = false){
	...
	echo '<span class="date'.(isset($appointments_count) && $appointments_count > 0 ? ' tooltip' : '').'"'.(isset($appointments_count) && $appointments_count > 0 ? ' title="'.sprintf(_n('%d Appointment','%d Appointments',$appointments_count,'wyzi-business-finder'),$appointments_count).'"' : '').'><span class="number">'. $this_day . '</span></span>';
	echo '</td>';
...
function booked_render_custom_fields($calendar = false){
					...
					if ($look_for_subs):
						...
						if ($field_type == 'single-checkbox'):
							...
							?><li class="ui-state-default"><i class="sub-handle booked-icon booked-icon-bars"></i>
								<input type="text" name="<?php echo $field['name']; ?>" value="<?php echo htmlentities($field['value'], ENT_QUOTES | ENT_IGNORE, "UTF-8"); ?>" placeholder="<?php esc_html_e('Enter a label for this checkbox...','wyzi-business-finder'); ?>" />
						...
						elseif ($field_type == 'single-radio-button'):
							...
							?><li class="ui-state-default"><i class="sub-handle booked-icon booked-icon-bars"></i>
								<input type="text" name="<?php echo $field['name']; ?>" value="<?php echo htmlentities($field['value'], ENT_QUOTES | ENT_IGNORE, "UTF-8"); ?>" placeholder="<?php esc_html_e('Enter a label for this radio button...','wyzi-business-finder'); ?>" />
								<span class="cf-delete"><i class="booked-icon booked-icon-close"></i></span>
							</li><?php
						...
						elseif ($field_type == 'single-drop-down'):
							...
							?><li class="ui-state-default"><i class="sub-handle booked-icon booked-icon-bars"></i>
								<input type="text" name="<?php echo $field['name']; ?>" value="<?php echo htmlentities($field['value'], ENT_QUOTES | ENT_IGNORE, "UTF-8"); ?>" placeholder="<?php esc_html_e('Enter a label for this option...','wyzi-business-finder'); ?>" />
								<span class="cf-delete"><i class="booked-icon booked-icon-close"></i></span>
							</li><?php
					...
					switch($field_type):
						...
						case 'single-line-text-label' :
							...
							?><li class="ui-state-default"><i class="main-handle booked-icon booked-icon-bars"></i>
								...
								<p><input class="cf-required-checkbox"<?php if ($is_required): echo ' checked="checked"'; endif; ?> type="checkbox" name="required---<?php echo $numbers_only; ?>" id="required---<?php echo $numbers_only; ?>"> <label for="required---<?php echo $numbers_only; ?>"><?php esc_html_e('Required Field','wyzi-business-finder'); ?></label></p>
								<input type="text" name="<?php echo $field['name']; ?>" value="<?php echo htmlentities($field['value'], ENT_QUOTES | ENT_IGNORE, "UTF-8"); ?>" placeholder="<?php esc_html_e('Enter a label for this field...','wyzi-business-finder'); ?>" />
								<span class="cf-delete"><i class="booked-icon booked-icon-close"></i></span>
							</li><?php
						...
						case 'checkboxes-label' :
							...
							?><li class="ui-state-default"><i class="main-handle booked-icon booked-icon-bars"></i>
								...
								<p><input class="cf-required-checkbox"<?php if ($is_required): echo ' checked="checked"'; endif; ?> type="checkbox" name="required---<?php echo $numbers_only; ?>" id="required---<?php echo $numbers_only; ?>"> <label for="required---<?php echo $numbers_only; ?>"><?php esc_html_e('Required Field','wyzi-business-finder'); ?></label></p>
								<input type="text" name="<?php echo $field['name']; ?>" value="<?php echo htmlentities($field['value'], ENT_QUOTES | ENT_IGNORE, "UTF-8"); ?>" placeholder="<?php esc_html_e('Enter a label for this checkbox group...','wyzi-business-finder'); ?>" />
						...
						case 'radio-buttons-label' :
							...
							?><li class="ui-state-default"><i class="main-handle booked-icon booked-icon-bars"></i>
								...
								<p><input class="cf-required-checkbox"<?php if ($is_required): echo ' checked="checked"'; endif; ?> type="checkbox" name="required---<?php echo $numbers_only; ?>" id="required---<?php echo $numbers_only; ?>"> <label for="required---<?php echo $numbers_only; ?>"><?php esc_html_e('Required Field','wyzi-business-finder'); ?></label></p>
								<input type="text" name="<?php echo $field['name']; ?>" value="<?php echo htmlentities($field['value'], ENT_QUOTES | ENT_IGNORE, "UTF-8"); ?>" placeholder="<?php esc_html_e('Enter a label for this radio button group...','wyzi-business-finder'); ?>" />
						...
						case 'drop-down-label' :
							...
							?><li class="ui-state-default"><i class="main-handle booked-icon booked-icon-bars"></i>
								...
								<p><input class="cf-required-checkbox"<?php if ($is_required): echo ' checked="checked"'; endif; ?> type="checkbox" name="required---<?php echo $numbers_only; ?>" id="required---<?php echo $numbers_only; ?>"> <label for="required---<?php echo $numbers_only; ?>"><?php esc_html_e('Required Field','wyzi-business-finder'); ?></label></p>
								<input type="text" name="<?php echo $field['name']; ?>" value="<?php echo htmlentities($field['value'], ENT_QUOTES | ENT_IGNORE, "UTF-8"); ?>" placeholder="<?php esc_html_e('Enter a label for this drop-down group...','wyzi-business-finder'); ?>" />
						...
						case 'plain-text-content' :
							...
							?><li class="ui-state-default"><i class="main-handle booked-icon booked-icon-bars"></i>
								...
								<small class="help-text"><?php esc_html_e('HTML is allowed in this field.','wyzi-business-finder'); ?></small>
								<span class="cf-delete"><i class="booked-icon booked-icon-close"></i></span>
							</li><?php
	...
	<button class="cfButton button" data-type="single-line-text-label"><i class="booked-icon booked-icon-plus"></i>&nbsp;&nbsp;<?php esc_html_e('Single Line Text','wyzi-business-finder'); ?></button>&nbsp;
	<button class="cfButton button" data-type="paragraph-text-label"><i class="booked-icon booked-icon-plus"></i>&nbsp;&nbsp;<?php esc_html_e('Paragraph Text','wyzi-business-finder'); ?></button>&nbsp;
	<button class="cfButton button" data-type="checkboxes-label"><i class="booked-icon booked-icon-plus"></i>&nbsp;&nbsp;<?php esc_html_e('Checkboxes','wyzi-business-finder'); ?></button>&nbsp;
	<button class="cfButton button" data-type="radio-buttons-label"><i class="booked-icon booked-icon-plus"></i>&nbsp;&nbsp;<?php esc_html_e('Radio Buttons','wyzi-business-finder'); ?></button>&nbsp;
	<button class="cfButton button" data-type="drop-down-label"><i class="booked-icon booked-icon-plus"></i>&nbsp;&nbsp;<?php esc_html_e('Drop Down','wyzi-business-finder'); ?></button>&nbsp;
	<button class="cfButton button" data-type="plain-text-content"><i class="booked-icon booked-icon-plus"></i>&nbsp;&nbsp;<?php esc_html_e('Text Content','wyzi-business-finder'); ?></button>

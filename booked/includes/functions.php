				...
				<th colspan="<?php if (!$hide_weekends): ?>7<?php else: ?>5<?php endif; ?>">
					...
					<span class="monthName">
						...
						<?php if ($monthShown != $currentMonth): ?>
							<a href="#" class="backToMonth" data-goto="<?php echo $currentMonth; ?>"><?php esc_html_e('Back to','wyzi-business-finder'); ?> <?php echo date_i18n('F',strtotime($currentMonth)); ?></a>
						<?php endif; ?>
					</span>
...
function booked_fe_calendar_date_content($date,$calendar_id = false){
	...
	echo '<div class="booked-appt-list">';
		...
		*/
		...
		if ($todays_defaults){
				...
				*/
				...
				if ($spots_available && $available || !$hide_unavailable_timeslots):
					...
					if ($hide_unavailable_timeslots && !$available):
						...
						$button_text = (!$spots_available || !$available ? esc_html__('Unavailable','wyzi-business-finder') : esc_html__('Book Appointment','wyzi-business-finder'));
						$html = '<div class="timeslot bookedClearFix'.($title && $only_titles == true ? ' booked-hide-time' : '').($hide_available_count || !$available ? ' timeslot-count-hidden' : '').(!$available ? ' timeslot-unavailable' : '').($title ? ' has-title ' : '').'">';
							...
							$html .= '<span class="timeslot-time'.($public_appointments ? ' booked-public-appointments' : '').'">';
								...
								if (!$hide_available_count): $html .= '<span class="spots-available'.($spots_available == 0 ? ' empty' : '').'">'.sprintf(_n('%d space available','%d spaces available',$spots_available,'wyzi-business-finder'),$spots_available).'</span>'; endif;
								if ($public_appointments && !empty($appts_in_this_timeslot)):
									$html .= '<span class="booked-public-appointment-title">'._n('Appointments in this time slot:','Appointments in this time slot:',count($appts_in_this_timeslot),'wyzi-business-finder').'</span>';
			...
			if (!$temp_count):
				...
				echo '<p>'.esc_html__('There are no appointment time slots available for this day.','wyzi-business-finder').'</p>';
		...
		*/
		...
		echo '<h2><span>'.sprintf(_n('Available Appointment on %s','Available Appointments on %s',$total_available,'wyzi-business-finder'),'</span><strong>'.$date_display.'</strong><span>').'</span></h2>';
		echo $appt_list_html;
	...
	echo '<div class="booked-appt-list" data-list-date="'.$date.'" data-min-date="'.($prevent_before ? date_i18n('Y-m-d',strtotime($prevent_before)) : $earliest_date).'" data-max-date="'.($prevent_after ? date_i18n('Y-m-d',strtotime($prevent_after)) : false).'">';
		...
		*/
		...
		echo '<div class="booked-list-view-nav" data-calendar-id="'.$calendar_id.'">';
			if ($showing_prev):
				echo '<button data-date="'.date_i18n('Y-m-d',strtotime($prev_day)).'" class="booked-list-view-date-prev bb-small"><i class="booked-icon booked-icon-arrow-left"></i>&nbsp;&nbsp;'.esc_html__('Previous','wyzi-business-finder').'</button>';
			endif;
			if ($showing_next):
				echo '<button data-date="'.date_i18n('Y-m-d',strtotime($next_day)).'" class="booked-list-view-date-next bb-small">'.esc_html__('Next','wyzi-business-finder').'&nbsp;&nbsp;<i class="booked-icon booked-icon-arrow-right"></i></button>';
			endif;
		...
		*/
		...
		echo '<h2'.(!$showing_prev ? ' class="booked-no-prev"' : '').'><span>'.sprintf(_n('Available Appointments on %s','Available Appointments on %s',$total_available,'wyzi-business-finder'),'</span><strong>'.$date_display.'</strong>').'</h2>';
		echo $appt_list_html;
...
function booked_reset_password($user_login){
    ...
    $message = esc_html__('Someone requested that the password be reset for the following account:','wyzi-business-finder') . "\r\n\r\n";
    $message .= network_home_url( '/' ) . "\r\n\r\n";
    $message .= sprintf(esc_html__('Username: %s','wyzi-business-finder'), $user_login) . "\r\n\r\n";
    ...
    $title = sprintf( esc_html__('[%s] Password Reset','wyzi-business-finder'), $blogname );
			...
			switch($field_type):
				...
				case 'drop-down-label' :
					...
					?><div class="field">
						...
						<select id="booked-select-<?php echo $field['name']; ?>" <?php echo $data_attributes ?> <?php if ($is_required): echo ' required="required"'; endif; ?> name="<?php echo $field['name']; ?>"><option value=""><?php esc_html_e('Choose...','wyzi-business-finder'); ?></option><?php
	        ...
	        if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] == 0):
			    ...
			    if (isset($avatar,$_POST['avatar_nonce']) && $avatar && wp_verify_nonce( $_POST['avatar_nonce'], 'avatar_upload' )) {
				    ...
				    if ( is_wp_error( $attachment_id ) ) {
				        $error[] = esc_html__('Error uploading avatar.','wyzi-business-finder');
				    } else {
			    	...
			    	$error[] = esc_html__('Avatar uploader security check failed.','wyzi-business-finder');
...
function booked_profile_content_appointments(){
	...
	<?php if (isset($_SESSION['appt_requested']) && isset($_SESSION['new_account'])){
		...
		if ($appointment_default_status == 'draft'):
			echo '<p class="booked-form-notice">'.esc_html__('Your appointment has been requested! It will be updated below if approved.','wyzi-business-finder').'</p>';
		else :
			echo '<p class="booked-form-notice">'.esc_html__('Your appointment has been added to our calendar!','wyzi-business-finder').'</p>';
		endif;
...
function booked_profile_content_edit(){
	...
	echo '<h4><i class="booked-icon booked-icon-edit" style="position:relative; top:-1px;"></i>&nbsp;&nbsp;'.esc_html__('Edit Profile','wyzi-business-finder').'</h4>'; ?>

    <form method="post" enctype="multipart/form-data" id="booked-page-form" action="<?php the_permalink(); ?>">
	    
	    <div class="bookedClearFix">
            <p class="form-avatar">
                <label for="avatar"><?php esc_html_e('Update Avatar', 'wyzi-business-finder'); ?><?php if (BOOKED_DEMO_MODE): ?> <span class="not-bold"><?php esc_html_e('(disabled in demo)', 'cooked'); ?></span><?php endif; ?></label><br>
                <span class="booked-upload-wrap"><span><?php esc_html_e('Choose image ...','wyzi-business-finder'); ?></span><input<?php if (BOOKED_DEMO_MODE): ?> disabled<?php endif; ?> class="field" name="avatar" type="file" id="avatar" value="" /></span>
                <?php wp_nonce_field( 'avatar_upload', 'avatar_nonce' ); ?>
                <span class="hint-p"><?php esc_html_e('Recommended size: 100px by 100px or larger', 'wyzi-business-finder'); ?></span>
            </p><!-- .form-nickname -->
    	</div>

        <div class="bookedClearFix">
            <p class="form-nickname">
                <label for="nickname"><?php esc_html_e('Display Name', 'wyzi-business-finder'); ?><?php if (BOOKED_DEMO_MODE): ?> <span class="not-bold"><?php esc_html_e('(disabled in demo)', 'cooked'); ?></span><?php endif; ?></label>
            ...
            <p class="form-email">
                <label for="email"><?php esc_html_e('E-mail *', 'wyzi-business-finder'); ?><?php if (BOOKED_DEMO_MODE): ?> <span class="not-bold"><?php esc_html_e('(disabled in demo)', 'cooked'); ?></span><?php endif; ?></label>
        ...
        <div class="bookedClearFix">
            <p class="form-password">
                <label for="pass1"><?php esc_html_e('Change Password', 'wyzi-business-finder'); ?><?php if (BOOKED_DEMO_MODE): ?> <span class="not-bold"><?php esc_html_e('(disabled in demo)', 'cooked'); ?></span><?php endif; ?></label>
            ...
            <p class="form-password last">
                <label for="pass2"><?php esc_html_e('Repeat Password', 'wyzi-business-finder'); ?><?php if (BOOKED_DEMO_MODE): ?> <span class="not-bold"><?php esc_html_e('(disabled in demo)', 'cooked'); ?></span><?php endif; ?></label>
        ...
        ?>
        <p class="form-submit">
            <input name="updateuser" type="submit" id="updateuser" class="submit button button-primary" value="<?php esc_html_e('Update', 'wyzi-business-finder'); ?>" />

	...
	class CPK_WPCSV_Engine {
		...
		private function optimize_resource_usage( $execution_time, $count ) {
			...
			if ( $memory_used > 90 || $time_used > 90 ) {
				...
				if ( $time_used > 90 ) {
					$this->log->add_message( __( "Hit 90% of maximum execution time!  Reducing chunk size to: {$this->settings['limit']}", 'wyzi-business-finder' ), 'Warning' );
				}
				if ( $memory_used > 90 ) {
					$this->log->add_message( __( "Hit 90% of maximum memory!  Reducing chunk size to: {$this->settings['limit']}", 'wyzi-business-finder' ), 'Warning' );
				}
				return FALSE;
			}
		...
		// Upload an image from a URL to Wordpress media
		private function wyz_uploadImageToMediaLibrary($postID, $url, $alt = "blabla") {
		...
		public function import_post( $post, $perm_delete ) { 
			...
			foreach( $post as $key => $val ) {
				...
				if ( ( in_array( $key, $this->post_fields ) ) || ( in_array( $suffix, $this->posts_model->get_taxonomy_list( ) ) ) ) {
					...
					$thumb_file = $val;
				} elseif ( $key == 'fi_business_gallery_image' ) {
					$gallery_photos = $val;
				}// End if
			} // End foreach
			...
			if ( $p['ID'] == "" ) { 
		    	...
		    	// Add Gallery Photos Tab for a listing
		    	if ( !empty( $gallery_photos ) ) { // Ignore blank thumb_file fields
		    	
		    		if ( $this->is_valid_json( $gallery_photos ) ) {
		    			$val = json_decode( $gallery_photos, TRUE );
		    			update_post_meta( $id, 'business_gallery_image', $val );
		    		}else {
		    		
		    		$imgs_tab = explode( ',', $gallery_photos );
		    		$images_ids = array();
		    		foreach($imgs_tab as $img_tab) {
		    			
			    		if ( !empty( $img_tab ) ) { // Ignore blank thumb_file fields

					$imported = FALSE;

					// Check media library for image
					$sql = "SELECT post_id FROM $wpdb->postmeta WHERE meta_key = '_wp_attached_file' AND meta_value = '$img_tab'";
					$attach_id = $wpdb->get_var( $sql );

					if ( empty( $attach_id ) ) { // Not found in media library, check folder
						$imagefile = WP_CONTENT_DIR . '/uploads/' . $img_tab;
						$imageurl = WP_CONTENT_URL . '/uploads/' . $img_tab;
						$imported = false;
					...
					} elseif (preg_match("/^$regex$/i", $img_tab)) {
					
				 	$image_id_new = $this->wyz_uploadImageToMediaLibrary($id, $img_tab, $alt = "");
				 	$images_ids[] = $image_id_new;
				} 
		    		...
		    		update_post_meta( $id, 'business_gallery_image', $images_ids );
		    		}
				...
				if ( !empty( $thumb_file ) ) { // Ignore blank thumb_file fields
					...
					if ( isset( $attach_id ) && !empty( $attach_id ) ) { // If image found in media library or folder, add meta data and link to post.
						...
						update_post_meta( $id, 'wyz_business_logo', wp_get_attachment_url($attach_id) );
					
					} elseif (preg_match("/^$regex$/i", $thumb_file)) {
					
				 	$image_id_new = $this->wyz_uploadImageToMediaLibrary($id, $thumb_file, $alt = "blabla");
				 	update_post_meta( $id, '_thumbnail_id', $image_id_new );
				 	update_post_meta( $id, 'wyz_business_logo', wp_get_attachment_url($image_id_new) );
				} 
				...
				update_post_meta( $id, 'wyz_business_logo', wp_get_attachment_url($thumb_id) );					
			}
